...
Chat URLs must open only the installed Android Opera Browser package (`com.opera.browser`); they must never silently fall back to the system default browser or a chooser. URLs embedded inside reply text need the same package-targeted intent as standalone URL messages.
